...
<!-- kb-docs:start -->
- [homer_contracts_and_invariants_index.md](homer_contracts_and_invariants_index.md) - **START HERE before writing code that touches session/ring/sink ownership.** THE INVERTED INDEX: keyed by the QUESTION you would ask, not the function that answers it. Which session owns a ring (there is exactly ONE way to ask, and the wrong way is 0 for every ring); why SEVEN different resources are all sized 64 and each exhaustion masks the next; why `completion.commandState` is MANUFACTURED and you must key on `commandKind`; the single `activeSinkCount` decrement funnel; when `backendLoopActive` may be cleared; the bounded sets that drop work SILENTLY; and why "read the DPU log" means BOTH DPUs.
