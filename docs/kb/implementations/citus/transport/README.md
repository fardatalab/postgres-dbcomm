...
- [dpu_collector_admission_starvation_mixed_workload.md](dpu_collector_admission_starvation_mixed_workload.md) - The mixed pgbench+basebackup hang: the control-mailbox fix (citus 6392a1853) and the connection-reuse hypothesis were BOTH refuted (causation run + static trace); the passes are procedural (pre-arming) and (after two static traces) the frontier is now farnet1's response-DELIVERY chain — static analysis exhausted, needs a live 4-frontier probe. Records eleven refuted causes/suspects and three defects fixed along the way.
