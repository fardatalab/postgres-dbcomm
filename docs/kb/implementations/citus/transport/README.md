...
- [dpu_collector_admission_starvation_mixed_workload.md](dpu_collector_admission_starvation_mixed_workload.md) - The mixed pgbench+basebackup hang, **RESOLVED** (citus 947b7692a): root-caused to the foreground recv-CQ liveness action being silently dropped by the 6-collector budget (foreground had no exact-demand recv path); fixed with exact recv demand for foreground peer-opens (Option B) and VALIDATED on the deterministic-hang recipe (non-pre-armed + SCALE=150) with positive mechanism confirmation (F2→F3→F4 now cross on the foreground result connections). Records twelve refuted causes/suspects and the full investigation trail (incl. a class-admission hardening fix + a blind-watchdog fix along the way).
