 ...
 * @file doca_types.h
 * @page doca_types
 * @defgroup DOCA_TYPES DOCA Types
 * @ingroup DOCACore
 ...
 * @struct doca_ip
 *
 * @brief Structs to hold IP address
 *
 *
 * @var doca_ip::ip
 * Array holding the IPV4 or IPV6 addr, for IPV4 we need to fill the last cell if the array ip[3]
 * for ipv6 we should fill all cells of the ip[4] array
 *
 * @var doca_ip::is_ipv4
 * Boolean flag to specify if IPV4 or IPV6, 1 incase of ipv4
 */
struct doca_ip {
	uint32_t ip[4];
	uint8_t is_ipv4;
