 ...
 * @file doca_dma.h
 * @page doca_dma
 * @defgroup DOCA_DMA DOCA DMA Engine
...
/**
 * @brief Retrieve the handle in the GPU memory space of a doca_dma
 *
 * @param [in] dma
...
/**
 * @brief Set if completions should be returned in submission order for the DMA context.
 *
 * @param [in] dma
 * doca_dma context set ordered completions on.
 * @param [in] ordered_completions
 * 1 if ordered completions should be enabled, 0 otherwise.
 *
 * @return
 * DOCA_SUCCESS - in case of success.
 * doca_error code - in case of failure:
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_dma_set_ordered_completions(struct doca_dma *dma, uint8_t ordered_completions);

/**
 * @brief Retrieve if this DMA context will return completions in submission order
 *
 * @param [in] dma
 * doca_dma context to get the property from.
 * @param [out] ordered_completions
 * 1 if ordered completions are enabled, 0 otherwise.
 *
 * @return
 * DOCA_SUCCESS - in case of success.
 * doca_error code - in case of failure:
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_dma_get_ordered_completions(struct doca_dma *dma, uint8_t *ordered_completions);
