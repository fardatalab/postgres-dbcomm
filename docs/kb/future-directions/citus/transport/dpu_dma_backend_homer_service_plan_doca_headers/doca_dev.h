...
/**
 * @brief Accelerates the reclaim process for a local device instance.
 *
 * This function optimizes the reclaim time by retaining critical resources in the cache upon closure,
 * allowing them to be reused when restarting.
 * It should be invoked before initiating device resource cleanup, including resources tied to the device,
 * such as DOCA Flow switch ports and their associated components.
 * For scenarios where a fast reclaim is needed, even in the event of a process crash, this function should
 * be called periodically (e.g., once per second) to ensure readiness.
 *
 * @param [in] dev
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_dev_accelerate_resource_reclaim(struct doca_dev *dev);
...
/**
 * @brief Get the PCI function type of a DOCA devinfo.
 *
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_get_pci_func_type(const struct doca_devinfo *devinfo, enum doca_pci_func_type *pci_func_type);
...
/**
 * @brief Get netdev SF index on the auxiliary bus for the device.
 *
 * @note Unlike doca_devinfo_rep_get_sf_index(), this function returns the SF index on the auxiliary bus
 *       (see "mlx5_core.sf.{sf_index}").
 *
 * @param [in] devinfo
 * The device to query.
 * @param [out] sf_index
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_get_sf_index(const struct doca_devinfo *devinfo, uint32_t *sf_index);
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_get_active_rate(const struct doca_devinfo *devinfo, uint64_t *active_rate);

/**
 * @brief Get vhca id of a device
 *
 * @param [in] devinfo
 * The device to query.
 * @param [out] vhca_id
...
/**
 * @brief Get uplink representor of IB-port on the device
 *
 * @param [in] devinfo
 * The device to query.
 * @param [out] ib_port
 * For PF device - returned uplink IB port index
 * For VF/SF device - returns 1
 * @return
 * DOCA_SUCCESS - in case of success.
 * Error code - in case of failure:
 * - DOCA_ERROR_INVALID_VALUE - received invalid input.
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_get_uplink_ib_port(const struct doca_devinfo *devinfo, uint16_t *ib_port);

/**
 * @brief Get netdev interface index for the device
 *
 * @param [in] devinfo
 * The device to query.
 * @param [out] iface_index
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_get_iface_index(const struct doca_devinfo *devinfo, uint32_t *iface_index);

/**
 * @brief Get data direct PCI device for the device
 *
 ...
 * The device to query.
 * @param [out] pci_addr_str
 * Returned data direct device PCI address, should be of size DOCA_DEVINFO_PCI_ADDR_SIZE at least.
 * @return
 * DOCA_SUCCESS - in case of success.
 * Error code - in case of failure:
 * - DOCA_ERROR_INVALID_VALUE - received invalid input.
 * - DOCA_ERROR_NOT_SUPPORTED - OS is note supported.
 * - DOCA_ERROR_NOT_FOUND - no data direct device was found.
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_get_data_direct_pci_dev(struct doca_devinfo *devinfo, char *pci_addr_str);
...
/**
 * @brief Check if notification moderation is supported for a device
 *
 * @details The notification moderation supported type: uint8_t*.
 *
 * @param [in] devinfo
 * The device to query.
 * @param [out] is_notification_moderation_supported
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_cap_is_notification_moderation_supported(const struct doca_devinfo *devinfo,
...
/**
 * @brief Get the accelerate resource reclaim capability of a DOCA devinfo.
 *
 * @details The accelerate resource reclaim property type: uint8_t*.
 *
 * @param [in] devinfo
 * The device to query.
 * @param [out] is_accelerate_resource_reclaim
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_cap_is_accelerate_resource_reclaim_supported(const struct doca_devinfo *devinfo,
...
/**
 * @brief Get the name of the ethernet interface of a DOCA devinfo_rep.
 *
 * @details The name of the ethernet interface is the same as it's name in ifconfig.
 * The name of the ethernet interface type: char[DOCA_DEVINFO_IFACE_NAME_SIZE].
 *
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_rep_get_iface_name(const struct doca_devinfo_rep *devinfo_rep,
					     char *iface_name,
					     uint32_t size);

/**
 * @brief Get netdev interface index of a DOCA devinfo_rep.
 *
 * @param [in] devinfo_rep
 * The representor of device to query.
 * @param [out] iface_index
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_rep_get_iface_index(const struct doca_devinfo_rep *devinfo_rep, uint32_t *iface_index);

/**
 * @brief Get representor IB-port of a DOCA devinfo_rep.
 *
 * @param [in] devinfo_rep
 * The representor of device to query.
 * @param [out] ib_port
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_rep_get_ib_port(const struct doca_devinfo_rep *devinfo_rep, uint16_t *ib_port);

/**
 * @brief Get host index of a DOCA devinfo_rep.
 *
 * @param [in] devinfo_rep
 * The representor of device to query.
 * @param [out] host_index
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_rep_get_host_index(const struct doca_devinfo_rep *devinfo_rep, uint32_t *host_index);

/**
 * @brief Get parent PCI Physical Function index of a DOCA devinfo_rep.
 *
 * @param [in] devinfo_rep
 * The representor of device to query.
 * @param [out] pf_index
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_rep_get_pf_index(const struct doca_devinfo_rep *devinfo_rep, uint32_t *pf_index);

/**
 * @brief Get PCI Virtual Function index of a DOCA devinfo_rep.
 *
 * @param [in] devinfo_rep
 * The representor of device to query.
 * @param [out] vf_index
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_rep_get_vf_index(const struct doca_devinfo_rep *devinfo_rep, uint32_t *vf_index);

/**
 * @brief Get SF number used on the craetion of the SF associated with the DOCA devinfo_rep.
 *
 * @note Unlike doca_devinfo_get_sf_index(), this function returns the SF number used when creating the SF
 *       (see "mlxdevm port add pci/{pci_addr} flavour pcisf pfnum {pf_num} sfnum {sfnum}" and
 *       "mlnx-sf --action create --device {pci_addr} --sfnum {sf_num} --hwaddr {mac_addr}").
 *
 * @param [in] devinfo_rep
 * The representor of device to query.
 * @param [out] sf_index
 ...
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_rep_get_sf_index(const struct doca_devinfo_rep *devinfo_rep, uint32_t *sf_index);

/**
 * @brief Get vhca id of a DOCA devinfo_rep.
 *
 * @param [in] devinfo_rep
 * Representor device info.
 * @param [out] vhca_id
...
/**
 * @brief Get the total number of VFs that the PF device represented by a DOCA devinfo_rep can support.
 *
 * @param [in] devinfo_rep
 * The representor device to query.
 * @param [in] total_vfs
 ...
 * Error code - in case of failure:
 * - DOCA_ERROR_INVALID_VALUE - received invalid input.
 * - DOCA_ERROR_NOT_PERMITTED - devinfo_rep represent a VF or a non-static PF.
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_rep_get_total_vfs(const struct doca_devinfo_rep *devinfo_rep, uint32_t *total_vfs);

/**
 * @brief Get the Vendor Unique ID of the PF associated with a representor DOCA devinfo.
 *
 * @details The Vendor Unique ID is used as stable ID.
 * The Vendor Unique ID type: char[DOCA_DEVINFO_REP_VUID_SIZE].
 *
 * @note If devinfo_rep represents a PF, the returned value is the PF's own VUID.
 *
 * @param [in] devinfo_rep
 * The representor device to query.
 * @param [out] pf_vuid
 * The Vendor Unique ID of the PF associated with devinfo_rep.
 * @param [in] size
 * The size of the pf_vuid buffer, must be at least DOCA_DEVINFO_REP_VUID_SIZE,
 * which includes the null terminating byte.
 *
 * @return
 * DOCA_SUCCESS - in case of success.
 * Error code - in case of failure. see doca_error_t.
 * - DOCA_ERROR_INVALID_VALUE - 'devinfo_rep' or 'pf_vuid' are NULL, or 'size' < DOCA_DEVINFO_REP_VUID_SIZE.
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_rep_get_pf_vuid(struct doca_devinfo_rep *devinfo_rep, char *pf_vuid, uint32_t size);

/**
 * @brief Get data direct PCI device for the device representor.
 *
 * @note This function is only supported on Linux and non-emulated representors.
 *
 * @param [in] devinfo_rep
 * The representor device to query.
 * @param [out] pci_addr_str
 ...
 * Error code - in case of failure:
 * - DOCA_ERROR_INVALID_VALUE - received invalid input.
 * - DOCA_ERROR_NOT_SUPPORTED - OS is note supported or representor is emulated.
 * - DOCA_ERROR_NOT_FOUND - no data direct device was found.
 */
DOCA_EXPERIMENTAL
doca_error_t doca_devinfo_rep_get_data_direct_pci_dev(struct doca_devinfo_rep *devinfo_rep, char *pci_addr_str);
