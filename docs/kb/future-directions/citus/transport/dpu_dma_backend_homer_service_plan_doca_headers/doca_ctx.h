 ...
 * @file doca_ctx.h
 * @page doca_ctx
 * @defgroup DOCA_CTX DOCA Context
 * @ingroup DOCACore
 ...
 *
 * @details In case the DOCA_TASK_SUBMIT_FLAG_FLUSH was not provided during doca_task_submit_ex() this method can
 * be used to flush the inflight tasks without the need to submit an additional task.
 *
 * @param [in] ctx
...
/**
 * @brief Set context completion vector
 *
 * @details A doca device has a set of completion vectors. Setting different completion vectors to different contexts
 * may spread the CPU load more evenly. DOCA recommends to use the same completion vector for all contexts that are
 * connected to a specific PE. However, Different doca devices may not map the same resources for the same completion
 * vector.
 *
 * @param [in] ctx
 * Context to set completion vector to
 * @param [in] comp_vector
...
/**
 * @brief Get number of completion vectors
 *
 * @param [in] devinfo
 * The device to query.
 * @param [out] num_comp_vectors
...
/**
 * @brief Set maximum number of NVTX ranges that can be displayed in parallel for a type of doca_task.
 *
 * @param [in] ctx
 * Context to set maximum number of NVTX ranges to
 * @param [in] num_ranges
 * Maximum number of NVTX ranges to set
 *
 * @return
 * DOCA_SUCCESS - in case of success.
 * Error code - in case of failure:
 * - DOCA_ERROR_INVALID_VALUE - received invalid input.
 * - DOCA_ERROR_BAD_STATE - The context is not idle
 * - DOCA_ERROR_NOT_SUPPORTED - NVTX profiling is not enabled.
 */
DOCA_EXPERIMENTAL
doca_error_t doca_ctx_profiler_set_max_nranges(struct doca_ctx *ctx, uint8_t num_ranges);
