...
- [dpu_readiness_collectors_and_continuation_edges.md](dpu_readiness_collectors_and_continuation_edges.md) — why everything is polled, what a completion queue actually buys (aggregation + a cookie slot), the ready queue as a cookie-less CQ, and the two missing continuation-graph edges. Read before resuming the async-continuation work.
