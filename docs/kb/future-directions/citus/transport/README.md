...
- [dpu_scheduler_arm_execute_mismatch.md](dpu_scheduler_arm_execute_mismatch.md) - Symptom map: ten sites where the DPU scheduler arms work from exact per-entity demand and then executes it with a linear scan on a type tag, ranked by hot-path x cost gap, plus the ready-queue audit (5 of 7 kinds dead) and named false positives. Sections 0/0b/0c carry the one CONFIRMED, FIXED instance: the setup listener that a live arena import starved forever, its three-latch diagnosis, and the landed S3.1b poll-due-obligation + reserved-admission fix with on-hardware cadence evidence.
...
- [homer_unified_session_core_plan.md](homer_unified_session_core_plan.md) - Plan to merge the two divergent Homer session abstractions (backend intent-driven RemoteExecutionSession + frontend HomerClientSession) into one frontend-safe portable core + transport vtable + thin backend/frontend adapters over the kept selected-DPU transport; carries the verified frontend-safety audit and the completion-model / reuse-pinning / peer-pairing reconciliation decisions. Gates S7.2.
