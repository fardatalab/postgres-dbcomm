...
typedef struct
{
	...
	uint64_t 	thread_wall_start_ns;
	uint64_t 	thread_wall_end_ns;
} VerifierState;
...
parse_and_verify_dump(VerifierState *state, const uint8_t *data, size_t data_len)
{
	...
	/* Record first work cycle for this thread */
	state->thread_wall_start_ns = monotonic_now_ns();
	...
	/* Record last work cycle for this thread */
	state->thread_wall_end_ns = monotonic_now_ns();
